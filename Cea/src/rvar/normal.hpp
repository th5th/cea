...
template <typename T>
class RVarNormal : public RVar<T>
{
    public:
        // Constructors
        RVarNormal(Prng * src, T m = 10, T s = 5)
        {
            set_params(m, s);
            set_source(src);
        }
    ...
    private:
        ...
        inline T rand_impl(std::true_type, std::false_type)
        {
            ...
            temp = (temp > 0.0 ? std::floor(temp+0.5) : std::ceil(temp-0.5));
        ...
        inline double ratio_of_uniforms()
        {
            ...
            } while (q > 0.27597 && (q > 0.27846 || v*v > -4.0*std::log(u)*u*u));

            return static_cast<double>(mean) + static_cast<double>(stddev) * (v/u);
        }
